...
- на странице видна версия вида `V1 · LOAD ... · HMR ...`
...
- baseline рамки подтверждён серией из 5 cold-start прогонов на статичной сцене
...
- поведение рамки при движении объекта ещё не считается верифицированным
...
- На текущем baseline это отдельно подтверждено 5 cold-start тестами подряд в `Safari`, каждый раз с новой `LOAD`-меткой.

### Что именно подтверждено тестами сейчас

- Проверялась статичная сцена с одним доминирующим круглым label.
- Каждый прогон делался заново:
  - открыть `Safari`
  - подтвердить камеру
  - дождаться живого кадра
  - снять скрин
  - закрыть `Safari`
- На 5 повторах рамка садилась в одно и то же место по кругу стабильно.

### Что ещё не подтверждено тестами

- Движение объекта в кадре.
- Быстрые смены масштаба/дистанции.
- Несколько похожих круглых объектов в одном кадре.
- Сценарии с частичным перекрытием label и сильными бликами.
